...
const Main = (props: IProps) => {
  ...
  const [absentsV, setAbsentsV]=useState <string> ("0")
  ...
  useEffect(() => {
    ...
    if (Number(absents)>=0) {
      // setFilteredList((oldState) => oldState.filter((std) => std.absents<=Number(absents)));
      setFilteredList((oldState) => oldState.filter((std) => std.absents<=Number(absents)));
      if(Number(absents)>=10)console.log("good");
      else{
  ...
  const handleRange=(event: React.ChangeEvent<HTMLInputElement>)=>{
    const absents=event.target.value
    setAbsentsV(absents)
    params.set("A", absents);
    setParams(params);
  }
  ...
  return (
    <div className="main-screen">
      ...
      <div className="filter">
        ...
        <div className="slider-absents">
          <h3>Absents</h3>
          <input type="range" min="0" max="100" value={absentsV} onChange={handleRange} style={{ width: "300px" }} />
          <h3>{absentsV}</h3>
        </div>
      </div>
      {filteredList.length ? (
